...
def solution(m, musicinfos):
    ...
    for i in musicinfos:
        temp = i.split(',')
        # 분계산
        mm = (int(temp[1][:2])*60+int(temp[1][3:])) - (int(temp[0][:2])*60+int(temp[0][3:]))

        temp_chord = _change(temp[3])
        temp_chord = temp_chord * (mm // len(temp_chord) + 1)
        temp_chord = temp_chord[:mm]

        if m in temp_chord:
            p1.append(temp[2])
            p2.append(len(temp_chord))

    return p1[p2.index(max(p2))]
